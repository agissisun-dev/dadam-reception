...
- **해피콜은 처방마다 두 번.** 기준은 약 수령 예정일. 1차 = 수령 +7일(복약 불편 확인), 2차 = 수령 + 일수 − 3일(남은 탕약·예약). 처방 12일 이하면 +7일 한 번만. **3차**: 마지막 예정 건(2차·단일)을 "연락함"으로 마쳤는데 재처방이 없으면 연락일 +7일에 한 번 더(3차는 더 안 만듦, 재처방·예약됨이면 같이 닫힘). `lib/happyCallRules.ts`
...
- **문자는 화면에서 보낸다**: 해피콜·주간 관리 줄의 [문자 보내기] → `app/api/sms`(서버) → 솔라피. 자동 예약 발송은 하지 않는다(사용자 결정). 키는 Vercel 서버 환경 변수 `SOLAPI_API_KEY`·`SOLAPI_API_SECRET`·`SMS_SENDER_NUMBER`, 설정은 `docs/setup-sms.md`.
...
- 오늘 할 일(기한 지난 것 포함)이 0이 되면 첫 화면에 축하 화면(색종이 + 탕약 봉투 캐릭터). 하루 한 번, 일이 있었던 날만. `lib/celebrationRules.ts`
