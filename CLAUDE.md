...
- **대체휴일**: 원래 공휴일이 토요일이면 대체휴일(월)은 **진료**, 일요일이면 휴진(사용자 결정 2026-09-22, 토요일 진료라서). `lib/holidays.ts`의 `substituteFor`.
- **휴진일(일요일·공휴일)에 걸린 일정은 그 전 진료일로 당긴다. 뒤로 미루지 않는다**(2026-09-22 사용자 결정: 2차 해피콜이 연휴 뒤로 밀리면 약이 끊긴 뒤라서). 일요일 → 토요일, 추석 연휴 9/24~27 → 모두 9/23. 해피콜·주간 관리·업무 모두 같은 규칙. 공휴일 표는 `lib/holidays.ts`(2026~2030, 2029~30 설·추석은 해 바뀔 때 대조). 달력은 휴진일을 회색 바탕·붉은 숫자·이름으로 표시. 이미 잡혀 있던 예정일은 안 옮김(예정일 옮기기로). 병원 임시 휴진은 표에 없음.
...
- 오늘 할 일(기한 지난 것 포함)이 0이 되면 첫 화면에 축하 화면(색종이 + 탕약 봉투 캐릭터). 0이 될 때마다(횟수 제한 없음, 사용자 결정 2026-09-18), 일이 있었던 날만. `lib/celebrationRules.ts`
