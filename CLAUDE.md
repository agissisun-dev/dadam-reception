...
- **휴진일(일요일·공휴일)에 걸린 일정은 가까운 진료일로 옮긴다**(2026-09-22 사용자 요청, "반올림하듯"). 앞뒤 거리가 같으면 해피콜·주간 관리는 뒤(월요일), 업무는 앞(토요일). 공휴일 표는 `lib/holidays.ts`(2026~2030, 2029~30 설·추석은 해 바뀔 때 대조). 달력은 휴진일을 회색 바탕·붉은 숫자·이름으로 표시. 이미 잡혀 있던 예정일은 안 옮김(예정일 옮기기로). 병원 임시 휴진은 표에 없음.
