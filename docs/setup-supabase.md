...
## 11. V3-2 3차 해피콜 허용 (2026-09-17 추가)
- [SQL Editor]에서 `supabase/schema-v3-2-happycall-round3.sql` 실행. `happy_calls`의 회차에 3이 허용된다.
- **앱을 새로 배포하기 전에 먼저 실행한다.** 실행 전에 새 앱에서 2차 해피콜을 "연락함"으로 저장하면 "3차 해피콜 만들기 실패"가 뜬다.
- 3차는 2차(또는 단일 건)를 연락했는데 재처방이 없을 때 연락일 +7일로 자동 생성. 재처방·예약됨으로 닫으면 3차도 같이 닫힌다.
