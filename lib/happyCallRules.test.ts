...
describe("planHappyCalls", () => {
  ...
  it("13일분: 두 건 (1차 +7, 2차 +10). +10이 일요일(9/20)이면 토요일로 당김", () => {
    expect(planHappyCalls("2026-09-10", 13).map((c) => c.due_date)).toEqual(["2026-09-17", "2026-09-19"]);
  });
  it("추석 연휴(9/24~27)에 걸리면 모두 연휴 전 9/23으로 당김 — 약 끊기기 전에 연락", () => {
    expect(planHappyCalls("2026-09-17", 30)[0].due_date).toBe("2026-09-23"); // 1차 9/24 → 9/23
    expect(planHappyCalls("2026-09-10", 20)[1].due_date).toBe("2026-09-23"); // 2차 9/27(일) → 9/23
  });
