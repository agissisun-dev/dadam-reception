...
describe("nextWeeklyDate", () => {
  ...
  it("휴진일에 걸리면 가까운 진료일로: 목요일 주간 관리가 추석(9/24)에 걸리면 9/23", () => {
    expect(nextWeeklyDate("2026-09-17", 4, 1, false)).toBe("2026-09-23");
    // 일요일(0) 주간 관리는 항상 월요일로
    expect(nextWeeklyDate("2026-09-16", 0, 1, true)).toBe("2026-09-21");
  });
  it("옮겨진 날짜에서 다음 주를 세도 원래 요일로 돌아온다", () => {
    // 9/23(수, 추석 때문에 옮겨진 것)에서 목요일 주간 → 다음은 10/1(목)
    expect(nextWeeklyDate("2026-09-23", 4, 1, false, "2026-09-23")).toBe("2026-10-01");
  });
