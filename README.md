...
| `/weekly` | 주간 관리 — 이번 주 보낼 환자(회차별 문구 자동), 발송함·건너뜀·답 없음·휴면, 원장 확인 대기 |
| `/templates` | 문구 틀 — 업무 안내문 5개, 주간 관리 문구 13개. 접수실이 직접 수정 |
...
환자 상세에서 [주간 관리 시작](요일·주기). 회차마다 다른 문구(질환 × 1~4주차, 5주차부터 4주차 반복)를 복사해 카카오 채널에서 사람이 보낸다. [발송함]이면 회차 +1. 환자 답변은 접수실이 기록하고 원장이 확인 대기 탭에서 지시를 남기면 다음 주 문구 옆에 보인다. 자동 발송·자동 휴면 없음.

## 다음

- 실사용 뒤 문구 틀 손보기, 답변 기록 진입점 추가, 재내원율 통계
